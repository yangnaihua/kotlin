...
class KotlinGradleProjectResolverExtension : AbstractProjectResolverExtension() {
    ...
    override fun populateModuleDependencies(gradleModule: IdeaModule,
        ...
        fun DataNode<out ModuleData>.getDependencies(): List<DataNode<out ModuleData>> {
            if (sourceSetByName == null) return emptyList()
            val externalSourceSet = sourceSetByName[data.id]?.second ?: return emptyList()
            return externalSourceSet.dependencies.mapNotNull { dependency ->
                when (dependency) {
        ...
        fun addTransitiveDependenciesOnImplementedModules() {
            ...
            for (currentModuleNode in moduleNodesToProcess) {
                ...
                val ideaProject = gradleModule.project
                ...
                while (toProcess.isNotEmpty()) {
                    ...
                    val ideaModule = if (moduleNodeForGradleModel != ideModule) {
                        ideaProject.modules.firstOrNull { it.gradleProject.path == moduleNodeForGradleModel?.data?.id }
                    }
                    else gradleModule
    ...
    val DataNode<out ModuleData>.sourceSetName
