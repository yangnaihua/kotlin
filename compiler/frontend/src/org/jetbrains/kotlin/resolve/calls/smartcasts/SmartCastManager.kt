...
class SmartCastManager {
    ...
    fun getSmartCastReceiverResult(
        ...
        return getSmartCastReceiverResultWithGivenNullability(receiverArgument, nullableParameterType, context)?.let {
    ...
    private fun getSmartCastReceiverResultWithGivenNullability(
        ...
        if (ArgumentTypeResolver.isSubtypeOfForArgumentType(receiverArgument.type, receiverParameterType)) return ReceiverSmartCastResult.OK

        return getSmartCastVariantsExcludingReceiver(context, receiverArgument).any {
            ArgumentTypeResolver.isSubtypeOfForArgumentType(it, receiverParameterType)
        }.let {
            if (it) ReceiverSmartCastResult.SMARTCAST_NEEDED_OR_NOT_NULL_EXPECTED else null
        }
    }
